...
const mockAdminProfile = {
  ...
  bio: 'Official Administrator Account - Mengelola sistem dan informasi kampus',
...
const SettingProfileAdmin = () => {
  const { user } = useAuth()
  const queryClient = useQueryClient()
  const { profile: localProfile, saveProfile, updateProfile } = useProfile('admin')
  ...
  useEffect(() => {
    if (localProfile) {
      ...
      setFormData({
        ...
        bio: mockAdminProfile.bio,
        website: mockAdminProfile.website,
        phone: mockAdminProfile.phone,
        username: mockAdminProfile.username
      })
    }
  }, [localProfile])
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(['public-profile', 'admin', formData.username])
    },
    onError: (error) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.name.trim()) {
      setSaveMessage('Nama Admin harus diisi')
      setTimeout(() => setSaveMessage(''), 3000)
      return
    }
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto p-6 sm:p-10 relative z-10">
          ...
          <motion.div 
            ...
            className="mb-12"
          >
            <span className="text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3 block">SYSTEM ADMINISTRATION</span>
            <h1 className="text-4xl md:text-5xl font-light text-lp-text tracking-tight mb-3">Pengaturan Profile Admin</h1>
            <p className="text-lg text-lp-text2 font-light">Kelola identitas administratif dan informasi publik sistem.</p>
          </motion.div>
              ...
              <div className="bg-white border border-lp-border rounded-[2.5rem] p-8 shadow-[0_32px_64px_rgba(0,0,0,0.03)] text-center">
                <div className="relative inline-block mb-6">
                  ...
                  <div className="absolute bottom-0 right-0 w-8 h-8 bg-lp-text text-white rounded-full flex items-center justify-center border-4 border-white">
                    <span className="text-[10px] font-bold italic">SYS</span>
                  </div>
                </div>
                
                <h3 className="text-2xl font-normal text-lp-text tracking-tight truncate">{formData.name || 'Administrator'}</h3>
                <p className="text-lp-text3 font-mono text-[11px] mb-8 mt-1 tracking-widest uppercase">@{formData.username || 'admin'}</p>
                
                <div className="space-y-3 pt-8 border-t border-lp-border">
                  <a
                    href={`/profile/admin/${formData.username || 'username'}`}
                    target="_blank"
            ...
            >
              <form onSubmit={handleSubmit} className="bg-white border border-lp-border rounded-[2.5rem] p-10 shadow-[0_32px_64px_rgba(0,0,0,0.03)]">
                <div className="space-y-10">
                  {/* Avatar Upload */}
                  <div>
                    <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-5">ADMINISTRATOR AVATAR</label>
                    <div className="flex flex-col sm:flex-row items-center gap-10">
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        ...
                        <p className="text-[11px] text-lp-text3 font-medium tracking-[0.05em] uppercase opacity-60">System Standard · PNG or JPG · Max 2MB</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                    <div className="group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">DISPLAY NAME *</label>
                      <input
                        type="text"
                        name="name"
                        value={formData.name}
                    ...
                    <div className="group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">ID / USERNAME *</label>
                      <input
                        type="text"
                        name="username"
                    ...
                    <div className="md:col-span-2 group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">SYSTEM EMAIL *</label>
                      <input
                        type="email"
                        name="email"
                    ...
                    <div className="md:col-span-2 group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">SYSTEM NOTICE / BIO</label>
                      <textarea
                        name="bio"
                        value={formData.bio}
                        onChange={handleChange}
                        ...
                        placeholder="Describe administrative role or system status..."
                        maxLength="150"
                      />
                    ...
                    <div className="group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">PORTAL URL</label>
                      <input
                        type="url"
                        name="website"
                        ...
                        onChange={handleChange}
                        className="w-full bg-lp-surface border border-lp-border rounded-xl p-4 text-lp-text text-[15px] font-normal focus:outline-none focus:border-lp-text transition-all"
                        placeholder="https://system.ac.id"
                      />
                    </div>

                    <div className="group">
                      <label className="block text-[11px] font-mono font-medium tracking-[0.2em] uppercase text-lp-text3 mb-3">SUPPORT LINE</label>
                      <input
                        type="tel"
                        name="phone"
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pt-10 border-t border-lp-border">
                    <p className="text-[10px] font-mono text-lp-text3 tracking-[0.15em] uppercase opacity-40 order-2 sm:order-1">
                      Version: {localProfile?.updated_at ? new Date(localProfile.updated_at).toLocaleString('en-US', { dateStyle: 'medium'}) : '1.0.0'}
                    </p>
...
export default SettingProfileAdmin